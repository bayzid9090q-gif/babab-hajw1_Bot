...
# Vercel file system is ephemeral.
    ...
    if context.args and context.args[0].isdigit():
        ...
        if ref != user.id:
            already = conn.execute(
                "SELECT 1 FROM referrals WHERE new_user=?", (user.id,)
            ).fetchone()
...
async def addfree_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" not in raw:
        await update.message.reply_text(
            "⚠️ ব্যবহার: /addfree Panel Name|https://example.com/panel"
        )
        return
...
async def addpaid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" not in raw:
        await update.message.reply_text(
            "⚠️ ব্যবহার: /addpaid VIP Panel|https://example.com/panel"
        )
        return
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = conn.execute(
        "SELECT coins, referrals, last_spin FROM users WHERE user_id=?", (uid,)
    ).fetchone()
