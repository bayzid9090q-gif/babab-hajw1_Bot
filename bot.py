...
async def send_join_message(message):
    ...
    await message.reply_text(
        "🌈 *WELCOME!*\n\n"
        "📢 আগে আমাদের Channel-এ Join করো।\n"
        "তারপর নিচের *Check Join* চাপো।",
    ...
    if context.args and context.args[0].isdigit():
        ...
        if ref != user.id:
            already = conn.execute(
                "SELECT 1 FROM referrals WHERE new_user=?",
                (user.id,),
            ).fetchone()
            ...
            if already is None:
                ensure_user(ref)
                conn.execute(
                    "INSERT INTO referrals(new_user, referrer) VALUES(?, ?)",
                    (user.id, ref),
                )
                conn.execute(
                    """
                    UPDATE users
                    SET referrals=referrals+1, coins=coins+5
                    WHERE user_id=?
                    """,
                    (ref,),
                )
...
async def addfree_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not name or not value:
        await update.message.reply_text("❌ Name এবং value দুটোই দিতে হবে।")
        return

    add_panel(name, "free", value)
    await update.message.reply_text(f"✅ Free Panel added: {name}")
...
async def addpaid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Paid Panel added: {name}")
...
async def delpanel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cur.rowcount:
        await update.message.reply_text(f"🗑️ Panel #{panel_id} deleted.")
    else:
        await update.message.reply_text("❌ এই ID-এর panel পাওয়া যায়নি।")
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = conn.execute(
        "SELECT coins, referrals, last_spin FROM users WHERE user_id=?",
        (uid,),
    ).fetchone()
    ...
    if query.data == "coins":
        ...
        await query.edit_message_text(
            f"👥 *REFERRAL*\n\n"
            f"প্রতি valid referral = 🪙 5 Coin\n"
            f"তোমার referral: `{refs}`\n\n"
            f"🔗 `{link}`",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=menu(),
        )
        ...
        conn.execute(
            """
            UPDATE users
            SET coins=coins+?, last_spin=?
            WHERE user_id=?
            """,
            (reward, date.today().isoformat(), uid),
        )
        conn.commit()
        ...
        if refs < 5 and coins < 100:
            await query.answer(
                "🔒 Paid Panel পেতে ৫ Referral অথবা ১০০ Coin লাগবে।",
                show_alert=True,
            )
            return
...
def create_app():
    ...
    app_builder = Application.builder().token(token).updater(None).build()
...
async def initialize_app():
    """Initialize PTB and configure Telegram's command/menu buttons."""
    if not os.environ.get("BOT_TOKEN"):
        raise ValueError("BOT_TOKEN Environment Variable is missing in Vercel settings!")
